...
describe('RoutineProgress', () => {
  ...
  it('should render active progress correctly', () => {
    ...
    expect(screen.getByText(/8.*of.*24/)).toBeInTheDocument();
  });
  ...
  it('should render inactive progress message', () => {
    ...
    expect(screen.getByText('Progress')).toBeInTheDocument();
    expect(
      screen.getByText(/this routine is not currently active/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /progress tracking will be available once you activate the routine/i
      )
    ).toBeInTheDocument();
  });
