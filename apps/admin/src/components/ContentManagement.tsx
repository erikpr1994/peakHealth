...
const mockEquipment = [
  {
    ...
    description: "Standard Olympic barbell for various strength training exercises",
    image: "https://images.unsplash.com/photo-1517963879433-488ba2d69ceb?w=300&h=200&fit=crop",
    status: "active",
    ...
    <Card className="hover:shadow-md transition-shadow">
      <CardHeader className="pb-3">
        <div className="flex items-center gap-3">
          ...
          <Badge variant={exercise.status === 'approved' ? 'default' : 'secondary'}>
            {exercise.status}
          </Badge>
        </div>
      </CardHeader>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {exercise.muscleGroups.map((muscle: string, index: number) => (
              <Badge key={index} variant="outline" className="text-xs">{muscle}</Badge>
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {exercise.equipment.map((equip: string, index: number) => (
              <Badge key={index} variant="outline" className="text-xs">{equip}</Badge>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {equipment.gyms.map((gym: string, index: number) => (
              <Badge key={index} variant="outline" className="text-xs">{gym}</Badge>
            ))}
          </div>
        </div>
        <div className="flex gap-2 pt-2">
    ...
    <div className="p-6 space-y-6">
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                ...
                <DialogDescription>
                  Create a new {activeTab.slice(0, -1)} by filling out the form below.
                </DialogDescription>
              </DialogHeader>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          <TabsTrigger value="exercises">Exercises ({mockExercises.length})</TabsTrigger>
          <TabsTrigger value="equipment">Equipment ({mockEquipment.length})</TabsTrigger>
          <TabsTrigger value="gyms">Gyms ({mockGyms.length})</TabsTrigger>
          <TabsTrigger value="categories">Categories ({mockCategories.length})</TabsTrigger>
        </TabsList>
                ...
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground capitalize">{category.type.replace('_', ' ')}</p>
                    </div>
