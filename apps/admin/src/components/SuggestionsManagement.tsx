...
const mockSuggestions = [
  {
    ...
    status: 'pending',
    description:
      'Suggest updating the instructions to include more safety details about proper form and breathing technique.',
    currentValue:
      'Stand with feet shoulder-width apart, lower into squat position...',
    suggestedValue:
      'Stand with feet shoulder-width apart, engage core, take a deep breath, then lower into squat position keeping knees aligned with toes...',
  ...
  },
  {
    ...
    status: 'pending',
    description:
      "Request to add resistance bands as they're commonly used for rehabilitation and home workouts.",
    suggestedValue:
      'Elastic resistance bands set with multiple resistance levels - light, medium, heavy. Ideal for strength training, physical therapy, and home workouts.',
  ...
  },
  {
    ...
    status: 'approved',
    description:
      'Update gym hours - they now stay open until midnight on weekends.',
  ...
  },
  {
    ...
    status: 'rejected',
    description:
      'Add Turkish Get-Up exercise for full-body functional training.',
    suggestedValue:
      'Complex full-body exercise starting from lying position with kettlebell, transitioning through multiple positions to standing.',
    ...
    rejectionReason:
      "Exercise already exists under 'Functional Training' category with different name.",
  },
];
...
export function SuggestionsManagement({
        ...
        {suggestion.status === 'approved' && suggestion.approvedBy && (
          <div className="p-3 bg-green-50 dark:bg-green-950 rounded-lg">
            <div className="flex items-center gap-2 text-sm text-green-700 dark:text-green-300">
              ...
              Approved by {suggestion.approvedBy} on{' '}
              {formatDate(suggestion.approvedAt)}
            </div>
          </div>
        )}
        ...
        {suggestion.status === 'rejected' && suggestion.rejectedBy && (
          <div className="p-3 bg-red-50 dark:bg-red-950 rounded-lg">
            <div className="flex items-center gap-2 text-sm text-red-700 dark:text-red-300 mb-1">
              ...
              Rejected by {suggestion.rejectedBy} on{' '}
              {formatDate(suggestion.rejectedAt)}
            </div>
  ...
  const pendingSuggestions = mockSuggestions.filter(
    s => s.status === 'pending'
  );
  const approvedSuggestions = mockSuggestions.filter(
    s => s.status === 'approved'
  );
  const rejectedSuggestions = mockSuggestions.filter(
    s => s.status === 'rejected'
  );
                ...
                <div className="flex items-center gap-3">
                  <Avatar>
                    <AvatarImage
                      ...
                      alt={selectedSuggestion.submittedBy}
                    />
                    <AvatarFallback>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {formatDate(selectedSuggestion.submittedAt)}
                    </p>
                  </div>
                </div>
                <Badge
