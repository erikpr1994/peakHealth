...
export const FEATURE_FLAG_CONFIG: FeatureFlagConfig = {
  monitoring: {
    enabled: process.env.NODE_ENV === 'production' && !!process.env.FEATURE_FLAG_MONITORING_ENABLED,
  ...
  analytics: {
    enabled: process.env.NODE_ENV === 'production' || process.env.FEATURE_FLAG_ANALYTICS_ENABLED === 'true',
    trackUsage: true,
    trackPerformance: true,
  },
};
